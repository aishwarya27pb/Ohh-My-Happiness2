  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      const { error: ssoError } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/auth/callback?flow=admin-oauth`,
        },
      });
      if (ssoError) {
    ...
    <div className="min-h-screen bg-[#1A1A1A] flex items-center justify-center px-4">
      ...
      >
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl font-semibold text-white bg-white/5 border border-white/10 hover:border-[#FFB449]/50 hover:bg-white/10 transition-all duration-300 shadow-sm disabled:opacity-50 mb-6"
        >
        ...
        {/* Divider */}
        <div className="my-6 flex items-center gap-3 mb-6">
